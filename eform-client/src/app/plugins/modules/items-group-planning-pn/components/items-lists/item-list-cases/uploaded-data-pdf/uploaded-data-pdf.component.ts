...
export class UploadedDataPdfComponent implements OnInit {
  ...
  @Output()
  onUploadedDataUploaded: EventEmitter<void> = new EventEmitter<void>();
  ...
  pdfFileUploader: FileUploader = new FileUploader({
    url: 'api/items-group-planning-pn/uploaded-data/pdf',
  });
  ...
  getAllUploadedData(itemCaseId: number) {
    this.itemsPlanningPnUploadedDataService
      .getAllUploadedData(itemCaseId)
      .subscribe((data) => {
